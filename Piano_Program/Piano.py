...
pygame.init()#On commence par initialiser tous les modules de la bibliothèque Pygame
pygame.mixer.init()#On initialise le module du son

root = Tk()#On créé une fenêtre ordinaire avec un titre
 
frame = Frame(root)#On créé un cadre suplémentaire
Frame(root).config(background="black")
frame.pack()#On importe le cadran

root.title('PIANO VIRTUEL')#On donne un titre à la fenêtre 

num1=StringVar()

topframe= Frame(root)#on créer la  fentre ou s'affichera les notes
topframe.pack(side = TOP)#On la situe dans la fenêtre
topframe.config(background="black")#On définit la couleur du bord du cadran des touches noires
...
button22=Button(topframe, state=DISABLED, height=6, width=5, padx=0, pady=0, relief=RIDGE)
button22.pack(side=LEFT)
